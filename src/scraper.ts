...
export class Scraper {
  ...
  /**
   * Fetch profiles a user is following using the v2 API.
   * @param userId The user ID whose following should be returned.
   * @param maxResults The maximum number of results to return.
   * @returns A promise that resolves to a paginator of following profiles.
   */
   ...
   * @param maxResults The maximum number of results to return.
   * @returns A promise that resolves to a paginator of users who retweeted the tweet.
   */
  public getTweetRetweeters(tweetId: string, maxResults: number) {
    return getTweetRetweeters(tweetId, maxResults, this.auth);
  }
   ...
   */
  public blockUserV2(userId: string) {
    return blockUserV2(userId, this.auth);
  }
   ...
   */
  public unblockUserV2(userId: string) {
    return unblockUserV2(userId, this.auth);
  }
